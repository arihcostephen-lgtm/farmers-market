...
<div class="card p-4">
    ...
    <?php if (count($documents) > 0): foreach ($documents as $document): $review = $reviewRows[$document['path']] ?? null; $url = '../uploads/docs/' . implode('/', array_map('rawurlencode', explode('/', $document['path']))); ?><tr>
        ...
        <td class="text-end text-nowrap"><a class="btn btn-sm btn-outline-secondary" href="<?php echo htmlspecialchars($url); ?>" target="_blank" rel="noopener">View</a>
        <form method="post" action="documents.php" class="d-inline">
            <input type="hidden" name="decision" value="approved">
            <input type="hidden" name="document_path" value="
            <?php echo htmlspecialchars($document['path'], ENT_QUOTES); ?>">
            <input type="hidden" name="review_document" value="1">
            <button class="btn btn-sm btn-success" value="approved" type="submit" data-review-document>Approve</button>
            <button class="btn btn-sm btn-outline-danger" value="rejected" type="submit" data-review-document>Reject</button>
        </form>
    </td>
    </tr>
    <?php endforeach; else: ?>
        <tr>
            <td colspan="7" class="text-center text-muted py-4">No admin-approved documents are awaiting supervisor review.</td>
        </tr>
        <?php endif; ?>
    </tbody>
</table>
