	...
	<div class="page-content">
		<?php  
			...
			if (isset($_GET['do']) && $_GET['do'] == "Approve") {
				...
				$approveSql = "UPDATE users SET status=1 WHERE user_id='$approveId' AND role=2";
				$approveQuery = mysqli_query($db, $approveSql);
			...
			if (isset($_GET['do']) && $_GET['do'] == "Reject") {
				$rejectId = (int) $_GET['uId'];
				$rejectSql = "UPDATE users SET status=0 WHERE user_id='$rejectId' AND role=2";
				$rejectQuery = mysqli_query($db, $rejectSql);
			...
			if ( $do == "Manage" ) { ?>
				...
				<?php  
					...
					$pendingFarmersQuery = mysqli_query($db, $pendingFarmersSql);
					$pendingFarmersCount = mysqli_num_rows($pendingFarmersQuery);
				?>
				<?php if ($pendingFarmersCount > 0) { ?>
						...
						<div class="card-body">
							...
							<div class="table-responsive">
								<table class="table table-sm align-middle mb-0">
									...
									<tbody>
										<?php while ($pendingFarmer = mysqli_fetch_assoc($pendingFarmersQuery)) { ?>
											<tr>
												<td><?php echo htmlspecialchars($pendingFarmer['user_name']); ?></td>
												<td><?php echo htmlspecialchars($pendingFarmer['user_email']); ?></td>
												<td><?php echo htmlspecialchars($pendingFarmer['user_phone']); ?></td>
												<td><?php echo htmlspecialchars($pendingFarmer['join_date']); ?></td>
												<td>
													<a href="users.php?do=Approve&uId=<?php echo $pendingFarmer['user_id']; ?>" class="btn btn-sm btn-success me-2">Approve</a>
													<a href="users.php?do=Reject&uId=<?php echo $pendingFarmer['user_id']; ?>" class="btn btn-sm btn-danger">Reject</a>
												</td>
											</tr>
										<?php } ?>
									</tbody>
		...
		<?php }
				...
				<div class="d-flex align-items-center justify-content-between mb-3">
					<h6 class="mb-0 text-uppercase">Manage Customers</h6>
					<a href="users.php?do=Add" class="btn btn-success">Add Customer</a>
				</div>
				<hr>
				<div class="card">
					<div class="card-body">
												      ...
												      <td>
												      	<?php  
												      		...
												      		<?php }
												      			...
												      			<span class="badge text-bg-info">CUSTOMER</span>
												      		<?php }
												      	?>
												      </td>
												      <td>
