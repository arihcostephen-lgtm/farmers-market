...
$productsSql = "SELECT p.*, c.cat_name, u.user_name AS farmer_name, f.farm_address FROM products p LEFT JOIN category c ON c.cat_id = p.category_id LEFT JOIN users u ON u.user_email COLLATE utf8mb4_unicode_ci = p.seller_email COLLATE utf8mb4_unicode_ci LEFT JOIN farmer f ON f.farm_email COLLATE utf8mb4_unicode_ci = p.seller_email COLLATE utf8mb4_unicode_ci WHERE p.status != 0 ORDER BY p.product_name ASC";
