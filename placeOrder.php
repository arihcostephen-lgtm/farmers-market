...
<body class="bg-light">
      ...
      <div class="card-body">
        ...
        <?php if ($product) { ?>
          ...
          <form method="post">
            ...
            <div class="form-text mb-3">Subtotal: UGX <span id="orderSubtotal"><?php echo number_format((float) $product['price'], 2, '.', ''); ?></span> · Tax: UGX <span id="orderTax">0.00</span> · Total: UGX <span id="orderTotal"><?php echo number_format((float) $product['price'], 2, '.', ''); ?></span></div>
  ...
  <script>
    const taxRate = <?php echo json_encode((float) (($taxRule['rate_percent'] ?? 0))); ?>;
    function updateOrderTotal() {
      const quantity = Number(document.getElementById('quantity')?.value || 0);
      const subtotal = quantity * <?php echo json_encode((float) ($product['price'] ?? 0)); ?>;
      document.getElementById('orderSubtotal').textContent = subtotal.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
      document.getElementById('orderTax').textContent = (subtotal * taxRate / 100).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
      document.getElementById('orderTotal').textContent = (subtotal * (1 + taxRate / 100)).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    }
