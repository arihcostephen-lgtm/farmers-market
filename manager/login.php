...
if (!empty($_SESSION['user_id']) && !empty($_SESSION['user_email']) && in_array((int) ($_SESSION['role'] ?? 0), [1, 4, 5], true)) {
    ...
    header('Location: ' . ($existingRole === 1 ? '../admin/dashboard.php' : ($existingRole === 5 ? '../supervisor/dashboard.php' : 'dashboard.php')));
    exit;
}
...
if (isset($_POST['managerSubmit'])) {
    ...
    if (!$query || mysqli_num_rows($query) === 0) {
        ...
        if ($row['user_password'] === $shaPass) {
            ...
            header('Location: ' . ($accountRole === 1 ? '../admin/dashboard.php' : ($accountRole === 5 ? '../supervisor/dashboard.php' : 'dashboard.php')));
            exit;
        } else {
