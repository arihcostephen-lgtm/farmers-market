...
$supervisorReports = mysqli_query($db, "SELECT r.report_id, r.title, r.report_body, r.created_at, COALESCE(u.user_name, r.supervisor_name, CONCAT('Supervisor #', r.supervisor_id)) AS supervisor_name, COALESCE(sp.region, 'Region not set') AS region FROM supervisor_reports r LEFT JOIN users u ON u.user_id = r.supervisor_id AND u.role = 5 LEFT JOIN supervisor_profiles sp ON sp.supervisor_id = r.supervisor_id ORDER BY r.created_at DESC, r.report_id DESC LIMIT 5");
$supervisorVisits = mysqli_query($db, "SELECT v.visit_id, v.visit_date, v.status, COALESCE(f.farm_name, CONCAT('Farm #', v.farm_id)) AS farm_name, COALESCE(u.user_name, CONCAT('Supervisor #', v.supervisor_id)) AS supervisor_name FROM farm_visits v LEFT JOIN farmer f ON f.farm_id = v.farm_id LEFT JOIN users u ON u.user_id = v.supervisor_id AND u.role = 5 ORDER BY v.visit_date DESC, v.visit_id DESC LIMIT 5");
...
$inquirySummaryQuery = mysqli_query($db, "SELECT COUNT(*) AS total_inquiries, COUNT(DISTINCT i.buyer_id) AS unique_customers, COUNT(DISTINCT p.seller_email) AS contacted_farmers FROM product_inquiries i LEFT JOIN products p ON p.product_id = i.product_id");
